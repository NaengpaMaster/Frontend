...
import { ShoppingItem, CATEGORIES, CATEGORY_EMOJIS, IngredientCategory, C } from '../data/mockData';
...
interface ShoppingListProps {
  ...
  onClearChecked: () => void;
  onMoveCheckedToFridge: () => void;
}

export function ShoppingList({ items, onToggle, onDelete, onAdd, onClearChecked, onMoveCheckedToFridge }: ShoppingListProps) {
  const [showAdd, setShowAdd] = useState(false);
  const [form, setForm] = useState({ name: '', quantity: '', category: '채소/과일' as IngredientCategory });
          ...
          Object.entries(grouped).map(([category, catItems]) => (
            <div key={category} style={{ marginTop: '16px' }}>
              <div style={{ fontSize: '10px', fontWeight: 700, letterSpacing: '0.12em', color: C.fgMuted, marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '5px' }}>
                <span>{CATEGORY_EMOJIS[category as IngredientCategory]}</span>
