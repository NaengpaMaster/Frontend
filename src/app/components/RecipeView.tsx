...
import { Ingredient, Recipe, RecipeComment, PresetIngredientItem, getRecipeMatch, getDaysUntilExpiry, C, User } from '../data/mockData';
...
interface RecipeViewProps {
  ...
  onAddComment: (comment: Omit<RecipeComment, 'id'>) => void;
}

type FilterMode = 'all' | 'available' | 'favorites';

function DifficultyDot({ difficulty }: { difficulty: Recipe['difficulty'] }) {
...
function RecipeDetail({
  ...
  recipe: Recipe;
  match: ReturnType<typeof getRecipeMatch>;
  currentUser: User;
  comments: RecipeComment[];
  presetIngredients: PresetIngredientItem[];
  onClose: () => void;
...
export function RecipeView({ recipes, ingredients, currentUser, comments, presetIngredients, onToggleFavorite, onAddRecipe, onUpdateRecipe, onDeleteRecipe, onAddComment }: RecipeViewProps) {
  ...
  const handleEditSave = (data: Omit<Recipe, 'id'>) => {
    ...
    <div className="recipe-shell" style={{ display: 'flex', flexDirection: 'column', background: C.bg }}>
      ...
      <div style={{ padding: '0 20px 14px', background: C.card, borderBottom: `1px solid ${C.border}` }}>
        <div style={{ display: 'flex', gap: '6px' }}>
          {([['all', '전체'], ['available', '80% 이상 가능'], ['favorites', '즐겨찾기']] as const).map(([val, label]) => (
            <button
              key={val}
