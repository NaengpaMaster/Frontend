...
interface FridgeManagerProps {
  ...
  onAdd: (ingredient: Omit<Ingredient, 'id' | 'addedDate'>) => void;
  onUpdate: (id: string, ingredient: Omit<Ingredient, 'id' | 'addedDate'>) => void;
...
function IngredientModal({
  ...
  const inputStyle = {
    ...
    padding: '10px 12px', color: C.fg, fontSize: '14px', outline: 'none', boxSizing: 'border-box' as const,
  };
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            <div>
              ...
              <textarea
                style={{ ...inputStyle, height: '52px', resize: 'none' } as React.CSSProperties}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '14px' }}>
          {[
            ...
            return (
              <button
                key={item.id}
                onClick={() => setMode(item.id as 'all' | 'partial')}
                style={{
                  padding: '12px 10px',
...
export function FridgeManager({ ingredients, presetIngredients, onAdd, onUpdate, onUse, onDelete }: FridgeManagerProps) {
  const [search, setSearch] = useState('');
  const [activeCategory, setActiveCategory] = useState<IngredientCategory | '전체'>('전체');
  const [showAdd, setShowAdd] = useState(false);
  const [editing, setEditing] = useState<Ingredient | null>(null);
  const [using, setUsing] = useState<Ingredient | null>(null);
