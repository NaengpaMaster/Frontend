...
import { Recipe, C, PresetIngredientItem } from '../data/mockData';
...
const DIFFICULTIES: Recipe['difficulty'][] = ['쉬움', '보통', '어려움'];
...
export function RecipeFormModal({ initial = {}, onSave, onClose, title = '레시피 등록', presetIngredients }: RecipeFormModalProps) {
  const [form, setForm] = useState({
    name: initial.name ?? '',
    category: initial.category ?? '기타',
    description: initial.description ?? '',
    cookTime: initial.cookTime ?? 15,
    difficulty: initial.difficulty ?? ('쉬움' as Recipe['difficulty']),
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {form.steps.map((step, idx) => (
                    <div key={idx} style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
                      ...
                      <textarea
                        style={{ ...inputStyle, flex: 1, resize: 'none', height: '70px' } as React.CSSProperties}
