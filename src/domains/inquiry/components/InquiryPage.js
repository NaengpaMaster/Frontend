...
import { Bot, CheckCircle, Clock, MessageSquare, Plus, Edit2, Trash2 } from 'lucide-react';
...
export function InquiryPage({ inquiries, currentUser, onFetchInquiries, onAddInquiry, onUpdateInquiry, onDeleteInquiry }) {
  ...
  const openInquiryFormFromChat = (lastQuestion) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100%', background: C.bg }}>
      ...
      {chatOpen && (
        <InquiryChatModal onClose={() => setChatOpen(false)} onOpenInquiry={openInquiryFormFromChat} />
      )}
    </div>
  );
}
